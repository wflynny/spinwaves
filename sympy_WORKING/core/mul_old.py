...
class Mul(AssocOp):
    ...
    def _eval_subs(self, old, new):
        ...
        # in case a term is a pow or a function, etc., try that class's subs first
        for o in terms_old:
            o = o.__class__(o._eval_subs(old,new))
        for s in terms_self:
            s = s.__class__(s._eval_subs(old,new))
                        ...
                        if (i+j) < noncomm_self_len and noncomm_self[i+j] == noncomm_old[j]:
                            ...
                            if j == noncomm_old_len-1:
                                ...
                                if not False in comms:
                                    ...
                                    noncomms_final = [noncomm_self[k] not in noncomm_old for k in range(noncomm_self_len)]
                                    comms_final = [comm_self[k] not in comm_old for k in range(comm_self_len)]
                                    expr = comms_final+noncomms_final
                                    return Mul(coeff_self/coeff_old, new, *expr)._eval_subs(old,new)
        ...
        # else the subexpression isn't in the totaly expression
            ...
            return self.__class_(*[s._eval_subs(old, new) for s in self.args])
