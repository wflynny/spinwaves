   ...
   Outputs:
     ...
     .perror
        ...
        *If* you can assume that the true reduced chi-squared value is unity --
        ...
        if fcn==None:
            raise UsageException("Usage: parms = mpfit('myfunt', ... )")
            return
        ...
        if self.damp != 0 and autoderivative == 0:
            raise KeywordsException('keywords DAMP and AUTODERIVATIVE are mutually exclusive!')
            return

        # Parameters can either be stored in parinfo, or x. x takes precedence if it exists
        if xall is None and parinfo is None:
            raise UsageException('must pass parameters in P or PARINFO!')
            return

        # Be sure that PARINFO is of the right type
        if parinfo is not None:
            if not isinstance(parinfo, list):
                raise ParInfoException('PARINFO must be a list of dictionaries!')
                return
            else:
            ...
            if xall is not None and len(xall) != len(parinfo):
                raise ParInfoException('number of elements in PARINFO and P must agree!')
                return
        ...
        if xall is None:
            ...
            if xall is None:
                raise UsageException('number of elements in PARINFO and P must agree!')
                return
        ...
        if np.any(qmin & qmax & (maxstep < minstep)):
            raise StepException('MPMINSTEP is greater than MPMAXSTEP!')
            return
        wh = np.where((qmin != 0.) | (qmax != 0.))[0]
        qminmax = len(wh > 0)
        
        # Finish up the free parameters
        ifree = np.where(pfixed != 1)[0]
        print ifree
        nfree = len(ifree)
        if nfree == 0:
            raise NoFreeParametersException('no free parameters!')
            return
        ...
        if limited is not None and limits is not None:
            ...
            if np.any((limited[:,0] & (xall < limits[:,0])) |
                ...
                raise ParameterVolumeException('parameters are not within PARINFO limits!')
                return
            if np.any((limited[:,0] & limited[:,1]) &
                                 (limits[:,0] >= limits[:,1]) &
                                 (pfixed == 0)):
                raise ParInfoException('PARINFO parameter limits are not consistent!')
                return
        ...
        if n < 0 or ftol <= 0 or xtol <= 0 or gtol <= 0 \
            ...
            raise KeywordsException('input keywords are inconsistent!')
            return

        if rescale != 0:
            raise ScalingInconsistencyException('DIAG parameter scales are inconsistent!')
            if len(diag) < n:
                return
            if np.any(diag <= 0):
                return
        ...
        if self.status < 0:
            raise FunctionFailureException('first call to "'+str(fcn)+'" failed!')
            return

        m = len(fvec)
        if m < n:
            raise ParameterVolumeException('number of parameters must not exceed data!')
            return
        self.dof = m-nfree
        self.fnorm = self.enorm(fvec)
        mpfit.mainLoop(self, fcn, x, xall, functkw, parinfo, ftol, xtol, gtol, maxiter, factor, 
                 nprint, fvec, step, dstep, dside, ifree, nfree, qulim, ulim, qanylim, 
                 qminmax, iterfunct, iterkw, nocovar, rescale, autoderivative, quiet, epsfcn)


    def mainLoop(self, fcn, x, xall, functkw, parinfo, ftol, xtol, gtol, maxiter, factor, 
                 nprint, fvec, step, dstep, dside, ifree, nfree, qulim, ulim, qanylim, 
                 qminmax, iterfunct, iterkw, nocovar, rescale, autoderivative, quiet, epsfcn):

        # Redefine some parameters to make the mainLoop call less bloated
        n = len(x)
        machep = self.machar.machep
        ...
        while(1):
            ...
            if nprint > 0 and iterfunct is not None:
                if (self.niter-1) % nprint == 0:
                    ...
                    if self.status < 0:
                        raise PrematureTerminationException('WARNING: premature termination by ' + str(iterfunt))
                        warnings.warn('premature termination by ' + str(iterfunct), Warning)
                        return

                    # If parameters were changed (grrr..) then re-tie
                    if np.amax(np.abs(xnew0-self.params)) > 0:
                        if self.qanytied:
                            self.params = self.tie(self.params, ptied)
                        x = self.params[ifree]
            ...
            if fjac is None:
                raise PrematureTerminationException('WARNING: premature termination by FDJAC2!')
                warnings.warn('premature termination by FDJAC2!', Warning)
                return

            # Determine if any of the parameters are pegged at the limits
            if qanylim:
                whlpeg = np.where(qllim & (x == llim))[0]
                nlpeg = len(whlpeg)
                whupeg = np.where(qulim & (x == ulim))[0]
                nupeg = len(whupeg)
            ...
            if self.fnorm != 0:
                for j in range(n):
                    ...
                    if wa2[l] != 0:
                        ...
                        gnorm = np.amax([gnorm,np.abs(sum0/wa2[l])])
            ...
            while(1):
                ...
                if qanylim == 0 and qminmax == 0:
                    ...
                    if qanylim:
                        ...
                        if nlpeg > 0:
                            wa1[whlpeg] = np.clip( wa1[whlpeg], 0., np.amax(wa1))
                        if nupeg > 0:
                            wa1[whupeg] = np.clip(wa1[whupeg], np.amin(wa1), 0.)

                        dwa1 = np.abs(wa1) > machep
                        whl = np.where((dwa1 != 0 & qllim) & (x+wa1 < llim))[0]
                        if len(whl) > 0:
                            t = ((llim[whl] - x[whl]) /
                                  wa1[whl])
                            alpha = np.amin([alpha, np.amin(t)])
                        whu = np.where((dwa1 != 0 & qulim) & (x+wa1 < ulim))[0]
                        if len(whu) > 0:
                            t = ((ulim[whu] - x[whu]) /
                                  wa1[whu])
                            alpha = np.amin([alpha, np.amin(t)])
                    ...
                    if qminmax:
                        nwa1 = wa1 * alpha
                        whmax = np.where((qmax != 0) & (maxstep > 0))[0]
                        if len(whmax) > 0:
                            mrat = np.amax(np.abs(nwa1[whmax]) /
                    ...
                    wh = np.where((qulim != 0) & (wa2 >= ulim1))[0]
                    if len(wh) > 0:
                        wa2[wh] = ulim[wh]
                    wh = np.where((qllim != 0.) & (wa2 <= llim1))[0]                    
                    if len(wh) > 0:
                        wa2[wh] = llim[wh]
                # endelse
                wa3 = diag * wa1
                ...
                if self.status < 0:
                    raise PrematureTerminationException('WARNING: premature termination by "'+fcn+'"!')
#                    self.errmsg = 'WARNING: premature termination by "'+fcn+'"'
                ...
                if ratio <= 0.25:
                    ...
                    if (0.1*fnorm1) >= self.fnorm or temp < 0.1:
                        temp = 0.1
                    delta = temp*np.amin([delta,pnorm/0.1])
                    par = par/temp
                else:
...
#                        'infinite; check model function for over- 'and underflow''')
        ...
        if self.fnorm is not None and fnorm1 is not None:
            self.fnorm = np.amax([self.fnorm, fnorm1])
    ...
    def fdjac2(self, fcn, x, fvec, step=None, ulimited=None, ulimit=None, dside=None,
        ...
        eps = np.sqrt(np.amax([epsfcn, machep]))
        m = len(fvec)
        n = len(x)
        ...
        if autoderivative == 0:
            ...
            if len(fjac) != m*nall:
                raise ImproperComputationException('Derivative matrix was not computed properly!')
#                print 'ERROR: Derivative matrix was not computed properly.'
     ...
     explicitly, and MPFIT does not.        
        ...
        for j in range(minmn):
            if pivot != 0:
                ...
                rmax = np.amax(rdiag[j:])
                kmax = np.where(rdiag[j:] == rmax)[0]
                ct = len(kmax)
                kmax = kmax + j
                if ct > 0:
                    kmax = kmax[0]
                ...
                for k in range(j+1, n):
                    ...
                    if a[j,lj] != 0:
                        ...
                        if pivot != 0 and rdiag[k] != 0:
                            temp = a[j,lk]/rdiag[k]
                            rdiag[k] = rdiag[k] * np.sqrt(np.amax([(1.-temp**2), 0.]))
                            temp = rdiag[k]/wa[k]
    ...
    def lmpar(self, r, ipvt, diag, qtb, delta, x, sdiag, par=None):
        ...
        nsing = n
        wa1 = qtb.copy()
        rthresh = np.amax(np.abs(np.diagonal(r))) * machep
        wh = np.where(np.abs(np.diagonal(r)) < rthresh)[0]
        if len(wh) > 0:
            nsing = wh[0]
            wa1[wh[0]:] = 0
        if nsing >= 1:
        ...
        paru = gnorm/delta
        if paru == 0:
            paru = dwarf/np.amin([delta,0.1])

        # If the input par lies outside of the interval (parl,paru), set
        # par to the closer endpoint

        par = np.amax([par,parl])
        par = np.amin([par,paru])
        if par == 0:
            par = gnorm/dxnorm
        ...
        while(1):
            ...
            if fp > 0:
                parl = np.amax([parl,par])
            if fp < 0:
                paru = np.amin([paru,par])
    ...
    def calc_covar(self, rr, ipvt=None, tol=1.e-14):
        ...
        if np.rank(rr) != 2:
            raise MatrixDimensionException('r must be a two-dimensional matrix!')
#            print 'ERROR: r must be a two-dimensional matrix'
            return -1
        s = rr.shape
        n = s[0]
        if s[0] != s[1]:
            raise MatrixDimensionException('r must be a square matrix!')
...
"""
All the exceptions. Each exception contains the strings of 
the old errmsg routine that it replaced. 
"""
class UsageException(Exception):
    "Usage: parms = mpfit('myfunt', ... )"
    'ERROR: either P or PARINFO(*)["value"] must be supplied.'
    'ERROR: must pass parameters in P or PARINFO'
    pass
class KeywordsException(Exception):
    'ERROR: input keywords are inconsistent'
    'ERROR: keywords DAMP and AUTODERIVATIVE are mutually exclusive'
    pass
class ParInfoException(Exception):
    'ERROR: PARINFO must be a list of dictionaries.'
    'ERROR: PARINFO parameter limits are not consistent'
    'ERROR: number of elements in PARINFO and P must agree'
    pass
class InfiniteValueException(Exception):
    """ ERROR: parameter or function value(s) have become 
        infinite; check model function for over- and underflow """
    pass
class PrematureTerminationException(Exception):
    """
    'WARNING: premature termination by ' + str(iterfunct)
    'WARNING: premature termination by FDJAC2'
    'WARNING: premature termination by "'+fcn+'"'
    """
    pass
class ParameterVolumeException(Exception):
    'ERROR: number of parameters must not exceed data'
    'ERROR: parameters are not within PARINFO limits'
    pass
...
class StepException(Exception):
    'ERROR: MPMINSTEP is greater than MPMAXSTEP'
    pass
class FunctionFailureException(Exception):
    """ 'ERROR: first call to "'+str(fcn)+'" failed' """
    pass
class ImproperComputationException(Exception):
    'ERROR: Derivative matrix was not computed properly.'
    pass
class NoFreeParametersException(Exception):
    'ERROR: no free parameters'
    pass
class ScalingInconsistencyException(Exception):
    'ERROR: DIAG parameter scales are inconsistent'
    pass