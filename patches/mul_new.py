...
class Mul(AssocOp):
    ...
    def _eval_subs(self, old, new):
        ...
        print 'oldcomm', comm_old,
        print 'oldnoncomm', noncomm_old
        print 'selfcomm', comm_self
        print 'selfnoncomm', noncomm_self
        ...
        if noncomm_old_len < noncomm_self_len:
                    ...
                    for j in range(noncomm_old_len):
                        ...
                        if (i+j) < noncomm_self_len and noncomm_self[i+j] == noncomm_old[j]:
                            ...
                            if j == noncomm_old_len-1:
                                # get rid of those noncommutative terms now. 
                                noncomms_final = noncomm_self[:]
                                for k in range(i,i+noncomm_old_len):
                                    noncomms_final.remove(noncomm_self[k])
                                noncomms_final.insert(i, new)
                                # check to make sure all of old's commutative terms are in the expression
                                comms = [comm_old[k] in comm_self for k in range(comm_old_len)]
                                if not False in comms:
                                    # Only now do we replace the whole subexpression with new         
                                    comms_final = comm_self[:]

                                    for ele in comm_old:
                                        if ele in comms_final: comms_final.remove(ele)

                                    expr = comms_final+noncomms_final
                                    return Mul(coeff_self/coeff_old, *expr)._eval_subs(old,new)
                                else: return self.__class__(*[s._eval_subs(old, new) for s in self.args])
                        else: break
        ...
        # but what if the noncommutative lists subexpression and the whole expression are both empty
            ...
            if not False in comms:
                ...
                return Mul(coeff_self/coeff_old,  new, *comms_final)._eval_subs(old,new)
            else:
                return self.__class__(*[s._eval_subs(old, new) for s in self.args])
